<?php
// update_institute_status.php - Update institute verification status (Admin access only)
...
if ($_SERVER['REQUEST_METHOD'] !== 'PUT' && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    echo json_encode([
        "status" => false,
        "message" => "Method not allowed. Use PUT or POST method."
    ]);
    exit;
}
...
try {
    ...
    $allowed_actions = ['approved', 'rejected', 'pending'];
    ...
    $checkStmt = $conn->prepare("SELECT id, user_name, email, role, is_verified FROM users WHERE id = ? AND role = 'institute'");
    ...
    if ($updateStmt->execute()) {
        ...
        throw new Exception("Failed to update institute verification status: " . $updateStmt->error);
    }
