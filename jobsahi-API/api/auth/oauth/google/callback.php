...
try {
    ...
    if ($existing_user) {
        ...
        if ($hasAuthProvider) {
            $insert_sql = "INSERT INTO users (user_name, email, password, phone_number, role, is_verified, status, google_id, auth_provider, last_activity) 
                           VALUES (?, ?, NULL, ?, ?, ?, 'active', ?, 'google', NOW())";
